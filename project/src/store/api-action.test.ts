...
describe('Async actions', () => {
  ...
  it('should dispatch fetchOffersAction when GET /hotels', async () => {
    ...
    await store.dispatch(getHotelsAction());

    const actions = store.getActions().map(({type}) => type);

    expect(actions).toEqual([
      getHotelsAction.pending.type,
      getHotelsAction.fulfilled.type
    ]);
  });
  ...
  it('should dispatch fetchFavoriteOffersAction when GET /favorites', async () => {
    ...
    await store.dispatch(getFavoriteHotelsAction());
    const actions = store.getActions().map(({type}) => type);

    expect(actions).toEqual([
      getFavoriteHotelsAction.pending.type,
      getFavoriteHotelsAction.fulfilled.type
    ]);
  });
});
